...
if __name__ == "__main__":
    ...
    # train("corpus.txt", True)
    model = build_model(vocab, 128)
    model.load_state_dict(torch.load("model/corpus.pth"))
    model.to(device)
    # print(model)
    sentence = "美国一直在炒作原油价纥"
    sentence_g = sentence[:-1]
    # print(sentence)
    sentences = []
    ppl_dict = {}
    with open("ge.txt", encoding="utf8") as f:
        for line in f.readlines():
            sentences.append(sentence_g+line.strip("\n"))
    # sentences = ["美国一直在炒作原油价格", "美国一直在炒作原油价哥", "美国一直在炒作原油价各"]
    # for s in sentences:
    #     ppl = calc_perplexity(sentence=s, model=model, vocab=vocab, window_size=6)
    #     print(ppl)
    # print(sentences)
    ppl_min = 999
    index_min = -999
    for index, sen in enumerate(sentences):
        ppl = calc_perplexity(sentence=sen, model=model, vocab=vocab, window_size=6)
        if(ppl < ppl_min):
            ppl_min = ppl
            index_min = index

    print("纠正：", sentences[index_min])
